...
def main():
    path = "D:/Umesh/2021/proto"
    list_path = get_all_protofile_path(path);

    for path in list_path:
        with open(path, 'r') as file:
            data = removeComments(file.read());
            #  print(removeComments(data))
            parse_result = proto.parse(data)
            messages = get_all_messages(data)

    print("In Main method")


def get_all_protofile_path(path):
    return glob.glob(path + "/*.proto")
